...
export async function createUser(input: Actor & { email: string; name: string; roles: RoleAssignment[] }) {
  ...
  const smtpConfig = await getTenantSmtpConfig(input.tenantId);
  const { delivered } = await sendMail({ to: email, ...passwordSetupEmail("th", { name: input.name, link: setupLink(rawToken), hours: 72 }) }, smtpConfig);
...
export async function issuePasswordSetupLink(input: Actor & { userId: string }) {
  ...
  const smtpConfig = await getTenantSmtpConfig(input.tenantId);
  const { delivered } = await sendMail({ to: ut.user.email, ...passwordSetupEmail(asLocale(ut.user.locale), { name: ut.user.name, link: setupLink(raw), hours: 72 }) }, smtpConfig);
...
export async function requestEmailChange(input: Actor & { userId: string; newEmail: string }) {
  ...
  const smtpConfig = await getTenantSmtpConfig(input.tenantId);
  const { delivered } = await sendMail({ to: newEmail, ...emailChangeEmail(asLocale(ut.user.locale), { name: ut.user.name, link: verifyLink(raw), hours: 24 }) }, smtpConfig);
  return { rawToken: raw, expiresAt, mailDelivered: delivered };
}
